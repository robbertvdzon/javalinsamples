...
import com.staticfiles.StaticFilesApplication;
...
class JavalinBaseApplicationIT {
    private StaticFilesApplication application;

    @BeforeEach
    void before() {
        application = new StaticFilesApplication();
        application.start();
    }
    ...
    @Test
    @DisplayName("Test get static file")
    void testStaticFile() {
        when()
                .get("/index.html")
                .then()
                .statusCode(200)
                .body(equalTo("Hello World from html file"));
